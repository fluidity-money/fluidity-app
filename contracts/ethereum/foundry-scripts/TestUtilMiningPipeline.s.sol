...
function strcmp(string memory a, string memory b) returns (bool) {
...
contract FakeUtilityGauges is IUtilityGauges {
    function getWeight(
        address token,
        string memory gauge
    ) external returns (uint256, uint256) {
        if (strcmp(gauge, "90% utility")) {
            return (9, 10);
        }
        if (strcmp(gauge, "10% utility")) {
            return (1, 10);
        }
        return (0, 10);
    }
    ...
    function vote(address, string memory, uint256) external {
        revert("not implemented");
    }
    ...
    function removeUtility(address, string memory) external {
        revert("not implemented");
    }
}

contract TestUtilGaugesPipeline is Script {
    ...
    function run() external {
        ...
        IFluidClient fluidGovToken = new FakeFluidToken(UtilityVars({
            poolSizeNative: 500_000 * 10**18,
            tokenDecimalScale: 10**18,
            exchangeRateNum: 10,
            exchangeRateDenom: 1,
        ...
        require(address(fluidGovToken) == vm.envAddress("FLU_ETHEREUM_FLUID_GOV_TOKEN_ADDR"), "fluid gov token address env is set incorrectly!");

        IUtilityGauges utilGauges = new FakeUtilityGauges();
        require(address(utilGauges) == vm.envAddress("FLU_ETHEREUM_UTILITY_GAUGES_ADDR"), "utility gauges address env is set incorrectly!");
        ...
        IFluidClient ninetyPercentUtility = new FakeFluidToken(UtilityVars({
            poolSizeNative: 0,
            tokenDecimalScale: 1,
            exchangeRateNum: 1,
            exchangeRateDenom: 1,
        ...
        IFluidClient tenPercentUtility = new FakeFluidToken(UtilityVars({
            poolSizeNative: 0,
            tokenDecimalScale: 1,
            exchangeRateNum: 1,
            exchangeRateDenom: 1,
        ...
        FluidityClientChange[] memory fluidClientChanges = new FluidityClientChange[](4);
        ...
        FluidityClientChange memory fluidChange2 = FluidityClientChange({
            ...
            overwrite: false,
            token: address(fluidToken),
            client: fluidGovToken
        });
        fluidClientChanges[1] = fluidChange2;

        FluidityClientChange memory fluidChange3 = FluidityClientChange({
            name: "90% utility",
            overwrite: false,
            token: address(fluidToken),
            client: ninetyPercentUtility
        });
        fluidClientChanges[2] = fluidChange3;

        FluidityClientChange memory fluidChange4 = FluidityClientChange({
            name: "10% utility",
            overwrite: false,
            token: address(fluidToken),
            client: tenPercentUtility
        });
        fluidClientChanges[3] = fluidChange4;
        r.updateUtilityClients(fluidClientChanges);
    }
}

contract TestUtilMiningPipeline is Script {
    function getPrivateKey(string memory key, uint32 index) public returns (uint) {
        return vm.deriveKey(key, index);
    }
