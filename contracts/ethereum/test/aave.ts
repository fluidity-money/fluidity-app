...
import { USDT_ADDR, AUSDT_ADDR, AAVE_POOL_PROVIDER_ADDR } from "../test-constants";
...
describe("token aave integration", async function () {
  ...
  it("should accrue interest", async function () {
    ...
    const initialPoolAmount = await token.callStatic.rewardPoolAmount();
    ...
    console.log(`aave earned ${finalPoolAmount.toNumber() - initialPoolAmount.toNumber()} interest over 100 blocks`);
  });
});
