...
// forked from https://github.com/valory-xyz/autonolas-governance/blob/main/contracts/veOLAS.sol
...
/*
 * Voting Escrow OLAS - the workflow is ported from Curve Finance Vyper implementation
 * Aleksandr Kuperman - <aleksandr.kuperman@valory.xyz>
...
/* This VotingEscrow is based on the FLUID token that has the following specifications:
*  - There is a cap of 1 billion tokens
* The maximum number of tokens for each year then can be calculated from the formula: 2^n = 1e27
* where n is the specified number of bits that is sufficient to store and not overflow the total supply.
* We limit n by 128, thus it would take 1340+ years to reach that total supply.
...
*
* The rest of calculations throughout the contract do not go beyond specified limitations.
*
...
string constant NAME = "veFLUID";

string constant SYMBOL = "VEFLU";

// Struct for storing balance and unlock time
// The struct size is one storage slot of uint256 (128 + 64 + padding)
struct LockedBalance {
    // Token amount. It will never practically be bigger. Initial FLUID cap is 1 bn tokens, or 1e27.
    uint128 amount;
...
contract VEGovToken is IErrors, IVotes, IERC20, IERC165 {
    ...
    // Maximum lock time (1 year)
    uint256 internal constant MAXTIME = 365 * 86400;
    // Maximum lock time (1 year) in int128
    ...
    // Voting token symbol
    string constant public symbol = SYMBOL;
