...
mod tests {
    ...
    fn test_wrap() {
        ...
        let starting_base_token = client
            .get_token_account_balance(&wrapping_user.base_token_account.unwrap())
            .unwrap();

        let wrap_accs = instructions::WrapInstructionAccountMetas {
        ...
        let new_base_token = client
            .get_token_account_balance(&wrapping_user.base_token_account.unwrap())
            .unwrap();

        assert_eq!(
            starting_fluid_token.amount.parse::<u64>().unwrap() + wrap_amt,
            new_fluid_token.amount.parse::<u64>().unwrap()
        );

        assert_eq!(
            starting_base_token.amount.parse::<u64>().unwrap() - wrap_amt,
            new_base_token.amount.parse::<u64>().unwrap()
        );
    }
}
