...
import (
	...
	"github.com/fluidity-money/fluidity-app/cmd/microservice-track-winners/lib"
)
...
func main() {
	...
	eventSignature := microservice_track_winners.HashEventSignature(
	...
	ethereum.Logs(func(log ethereum.Log) {
		...
		winner, err := microservice_track_winners.DecodeWinner(
			logAddress,
			transactionHash,
			winnerAddressPadded,
			winningAmountPadded,
