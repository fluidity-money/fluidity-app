...
const ConnectWalletModal = ({ height, width, enable, toggle }: { height?: string; width?: string; enable: boolean; toggle: Function; }) => {
  ...
  const renderedOptions = options.map(({ name, src, disabled, connectorId }, index) => {
    const isConnected = wallet.status === "connected" && connectorId === wallet.connector
    return (
      <Button
        label={name}
        key={name + index}
        ...
        //fontSize="font-large"
        icon={<img src={src} className={`wallet-icon ${(disabled ?? "") && "disabled"}`} alt={name} />}
        goto={() => { !disabled && connectWallet(connectorId) }}
  ...
  return (
    ...
    >
      <div className="connect-modal-body">
        <h2 className="primary-text">{wallet.status === "connected" ? "Wallet Connected" : "Connect to Wallet"}</h2>
