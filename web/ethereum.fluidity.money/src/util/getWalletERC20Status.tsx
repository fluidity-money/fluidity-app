...
import contractList, { SupportedFluidContracts, SupportedContracts } from "util/contractList";
...
const getWalletERC20Status = async(signer: Signer) => {
    ...
    await Promise.all(extOptions.map(async (value: TokenKind) => {
        const {decimals} = contractList.ETH?.[value.type as SupportedContracts] || {};
        if (!decimals) return;
        // Gets amount
        const amount = await(getBalanceOfERC20(value.type as SupportedContracts, signer, decimals))
    ...
    await Promise.all(intOptions.map(async(value: TokenKind) => {
        ...
        if (!decimals) return;
        // Gets amount
        const amount = await(signer! && getBalanceOfERC20(value.type as SupportedFluidContracts, signer, decimals))
        if(parseUnits(amount).gt(0)) {
            const renderedType: walletDataType = ({
                type: value.type,
                amount: amount,
