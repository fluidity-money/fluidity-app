...
import contractList, {SupportedContracts} from 'util/contractList';
...
const makeContractSwap = async (type: 'toFluid' | 'fromFluid', tokenAmount: string, contractName: SupportedContracts, signer: Signer | null, addError: (error: string) => void) => {
    ...
    const decimals = contractList['ETH']?.[contractName]?.decimals
    if (!decimals) return;

    const amountTrimmed = decimalTrim(tokenAmount, decimals + 1);
    const amount = ethers.utils.parseUnits(amountTrimmed, decimals)

    if (amount.eq(0)) {
        addError(`Cannot send 0 ${contractName}!`);
        return;
    }
    if (amount.lt(0)) {
        addError(`${amountTrimmed} is less than 0!`);
        return;
    }
