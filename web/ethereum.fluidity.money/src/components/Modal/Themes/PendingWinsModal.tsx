...
const PendingWinsModal = ({
  ...
  const sendPendingWin = async (hash: string) => {
    ...
    const {signature} = await apiPOSTBody('/manual-reward', {transaction_hash: hash})
    ...
    >
      <div className="connect-modal-body">
        ...
        <div style={{color: "white"}}>
          The following wins will be automatically redeemed at some point, but you can redeem them manually now by clicking on one
        </div>
        <div className="connect-modal-form">{pendingWins.map(win => 
          <div onClick={() => sendPendingWin(win.transaction_hash)} style={{color: "white"}}> 
            {win.transaction_hash + " "}
            {win.from_address + " "}
            {win.to_address + " "}
            {win.token_details.token_short_name + " "}
            {formatAmount(win.winning_amount, win.token_details.token_decimals, 2) + " "}
          </div> 
        )}</div>
      </div>
    </GenericModal>
