import {networkToChainId, NETWORK_TO_CHAIN_ID, SOL, Token} from "@saberhq/token-utils";
...
const useFluidTokens = () => {
  ...
  useEffect(() => {
    ...
    const updatedList: FluidTokens = tokenList.reduce((allTokens, 
      {symbol, mintAddress, name, decimals}) => (
        {...allTokens, [symbol]: symbol === 'SOL' ? SOL.devnet : new Token ({
        chainId: NETWORK_TO_CHAIN_ID.devnet,
