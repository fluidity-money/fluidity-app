...
type submitUniquePhraseResponse = {
  response_message: 'Bad network!' | 'Bad address!' | 'Phrase already received!',
  was_error: boolean,
  unique_phrase: string
};

export const submitUniquePhraseRequest = (network: string, address: string)
...
export const handleUniquePhraseRequest = async (
  ...
  try {
    ...
    const response = await submitUniquePhraseRequest(network, address);
