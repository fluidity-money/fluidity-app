...
type FluidityInstructionCtor = {TokenSymbol: string, bumpSeed: number} & ({Wrap: TokenAmount, Unwrap?: null} | {Wrap?: null, Unwrap: TokenAmount});
...
export class FluidityInstruction extends SolanaEnum {
   ...
   */
  static async getBumpSeed(TokenSymbol: string, programId: PublicKey): Promise<number> {
   ...
   * @returns the account 
   */
  static async getProgramAddress(TokenSymbol: string, programId: PublicKey): Promise<PublicKey> {
    const seedString = `FLU:${TokenSymbol}_OBLIGATION`;
    const seedBuffer = Buffer.from(seedString, 'utf8');
