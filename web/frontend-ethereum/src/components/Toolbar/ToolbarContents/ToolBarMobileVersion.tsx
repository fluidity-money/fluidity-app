...
const ToolBarMobileVersion = ({
    ...
    const address = `${wallet.account}`.substr(0, 6) + "..." + `${wallet.account}`.substr(`${wallet.account}`.length - 4, `${wallet.account}`.length - 1);
    ...
    return (
        <div className="toolbar-container-mobile flex-space-between">
            ...
            {/* Modals */}
            {active
                ? <WalletConnectedModal enable={toggle} toggle={modalToggle} wallet={wallet} address={address} />
