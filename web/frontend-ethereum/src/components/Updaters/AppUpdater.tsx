...
import {ContractInfo, getAllFluidContracts, ListenerArgs} from "util/contractUtils";
        ...
        contracts.map(c => {
            c.contract.addListener('Transfer', ((...args: ListenerArgs) => {
                ...
                if (address === receiver && sender !== zeroAddr)
                    addNotification(`Received ${formatUnits(amount, c.decimals).toString()} ${c.name} from ${trimAddress(sender)}`);
            }))
        });
    }
