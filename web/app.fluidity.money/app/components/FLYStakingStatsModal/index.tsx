...
const FlyStakingStatsModal = ({
  visible,
  close,
  showConnectWalletModal,
  shouldUpdateFlyBalance,
  staking = true,
}: FlyStakingStatsModalProps) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!seconds || seconds.eq(new BN(0))) setFlyUnstakable(flyUnstaking);
    })();
  ...
  const setMaxBalance = () => {
    if (isStaking) {
      setSwapInput(
        addDecimalToBn(
          snapToValidValue(flyBalance.toString(), FlyToken, flyBalance),
          FlyToken.decimals
        )
      );
    } else {
  ...
  const unstakeAmount: BN = snapToValidValue(
    swapInput,
    FlyToken,
    new BN(flyStaked.toString())
  );
      ...
      createPortal(
        <>
          ...
          >
            ...
            ></div>
            <div
              className={`fly-staking-stats-modal-container ${
                visible === true ? "show-fly-modal" : "hide-modal"
              }`}
            >
                      ...
                      <div className="fly-staking-stats-modal-row">
                        <Card border="solid" fill>
                          <Text className="center-text" size="lg" prominent>
                            💸 Stake your $FLY to earn Airdrop Rewards and
                            [REDACTED] in Superposition (SPN) 🐱
                          </Text>
                        </Card>
                      </div>
                      <div
                        style={{ gap: "0.5em", flexDirection: "column" }}
                        className="fly-staking-stats-modal-row"
                      >
                          ...
                          <Card fill>
                            <Hoverable
                              ...
                              tooltipContent={
                                <div className="flex-column">
                                  <Text className="staking-stats-info-text">
                                    The amount of $FLY Token you have staked
                                    from your total $FLY Balance.
                                  </Text>
                                </div>
                              }
                            >
                          ...
                          <Card className="fly-staking-grid-item-long" fill>
                            ...
                            >
                              ...
                              <GeneralButton
                                ...
                                layout="after"
                                disabled={
                                  flyUnstaked || flyUnstaking.eq(new BN(0))
                                }
                                handleClick={finaliseUnstake}
                                className={`fly-unstaking-claim-action-button ${
                                  flyUnstaked ? "claim-button-staked" : ""
                                }`}
                              >
                                <Text
                                  size="md"
                    ...
                    <div className="fly-submit-claim-modal-options">
                      {isStaking ? (
                        <div className="fly-staking-stats-modal-row">
                          <StakingWarning
                            header={
                              <Text size="lg" bold prominent className="black">
                                Staking will lock up your funds for{" "}
                                <span className="underline">7 Days</span>.
                              </Text>
                            }
                            body={
                              <Text size="md" className="black">
                                To have access to your staked funds once again,
                                you must go through the process of unstaking
                              </Text>
                            }
                          />
                        </div>
                            ...
                            <StakingWarning
                              ...
                              body={
                                <Text size="md" className="black">
                                  ...
                                  receive staking rewards and terminate the
                                  un-bonding process.
                                </Text>
                              }
                            />
                      ...
                      <div className="fly-staking-stats-modal-row">
                        <div className="fly-staking-input-container">
                          ...
                          <div className="staking-input-lower">
                            {isStaking
                              ? `${getValueFromFlyAmount(
                                  flyBalance.sub(stakeAmount)
                                )} $FLY remaining`
                              : `${getValueFromFlyAmount(
                                  new BN(flyStaked.toString()).sub(
                                    unstakeAmount
                                  )
                                )} $FLY staked remaining`}
                            <div className="flex" style={{ gap: "0.5em" }}>
                              {isStaking ? (
                                <Text size="md">
                                  Staking {getValueFromFlyAmount(stakeAmount)}{" "}
                                  $FLY
                                </Text>
                              ) : (
                                <Text size="md">
                                  Unstaking{" "}
                                  {getValueFromFlyAmount(unstakeAmount)} $FLY
                                </Text>
                              )}
                      ...
                      <Text className="fly-once-approved">
                        Once approved, transactions cannot be reverted. By
                        pressing send you agree to our{" "}
                        <a
                          className="link"
                          onClick={() => {
                    ...
                    <div className="fly-submit-claim-modal-options">
                        ...
                        <div className="fly-staking-stats-modal-row">
                          <div className="fly-points-info-container">
                            <div className="flex">
                              ...
                              <Text size="lg" holo>
                                Staking $FLY will reward you points.
                              </Text>
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="fly-staking-stats-modal-row">
                        ...
                        <div className="flex-column">
                          <Text size="lg" prominent>
                            {isStaking
                              ? `Stake $FLY ${getValueFromFlyAmount(
                                  stakeAmount
                                )}`
                              : `Unstake $FLY ${getValueFromFlyAmount(
                                  unstakeAmount
                                )}`}
                          </Text>
                          {currentStatus >= State.HasStaked &&
                    ...
                    {currentStatus === State.Stats ? (
                      ...
                      <GeneralButton
                        ...
                        disabled={
                          currentStatus === State.HasStaked ||
                          (isStaking
                            ? stakeAmount.eq(new BN(0))
                            : unstakeAmount.eq(new BN(0)))
                        }
                        handleClick={() => handleClick(isStaking)}
                        className={`fly-staking-stats-action-button ${
                          currentStatus === State.HasStaked - 1 ? "rainbow" : ""
                        } ${
                          currentStatus === State.HasStaked
                            ? "claim-button-staked"
                            : ""
