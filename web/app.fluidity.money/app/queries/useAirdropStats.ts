...
const queryAirdropStatsByAddress= gql`
  query AirdropStats($address: String!, $now: timestamp!) {
    ...
    referralsCount: lootbox_referrals_aggregate(where: { referrer: { _eq: $address } }) {
      aggregate {
        count
      }
    }
...
export const useAirdropStatsByAddress = async (address: string) => {
  ...
  return await jsonPost<ExpectedAirdropStatsByAddressBody, ExpectedAirdropStatsByAddressResponse>(
    url,
    body,
