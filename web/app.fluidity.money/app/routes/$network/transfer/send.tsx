import { Card, Display, Heading, useViewport, Video, Text, GeneralButton, Form, TokenIcon, numberToMonetaryString, LoadingDots } from "@fluidity-money/surfing";
...
const Send = () => {
  ...
  // Select Your Asset
  const [selectedAsset, setSelectedAsset] = useState<AugmentedAsset | undefined>(deeplinkAssetToken)
  ...
  useEffect(() => {
    ...
    const balanceNumber = getUsdFromTokenAmount(selectedAsset.userTokenBalance, selectedAsset.decimals, 1)
    ...
    if (amountRemaining > 0) {
      ...
      setInputHint(`${amountRemaining.toFixed(2)} ${selectedAsset.symbol} remaining (${numberToMonetaryString(amountRemaining)})`)
    } else {
      setValidAmount(false)
  ...
  // Recipient Address
  const [recipientAddress, setRecipientAddress] = useState(deeplinkAddress || "");
  ...
  useEffect(() => {
    ...
    setCanSend(true)
  }, [selectedAsset, recipientAddress, amountToSend, validAddress, validAmount])
  ...
  const handleSubmit = async () => {
    ...
    try {
      const txValue = ethers.utils.parseUnits(amountToSend, selectedAsset.decimals);
      const tokenContract = new ethers.Contract(selectedAsset.contract_address, tokenAbi, signer);
      const transferData = tokenContract.interface.encodeFunctionData('transfer', [recipientAddress, txValue]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const gasLimit = await tokenContract.estimateGas.transfer(recipientAddress, txValue);
          ...
          />
          {
            selectedAsset && (
              <>
                ...
                >
                  <Text className="send-table-header" prominent bold size="lg">Potential Rewards</Text>
                  <Text>Top prize</Text>
                ...
                <div className="send-table fees">
                  <Text className="send-table-header" size="md">Fees</Text>
                  <Text>Network fee</Text>
                  <Text>{numberToMonetaryString(0)}</Text>
                  <div className="rewards-table-rule" />
                  <Text>Gas fee</Text>
                  <Text>{transactionFee || <div style={{ transform: 'scale(0.5)' }}><LoadingDots /></div>}</Text>
                </div>
                <Text>
                  Once sent, transactions cannot be reverted. By pressing send you agree to our <a className="send-terms-link" href="https://static.fluidity.money/assets/fluidity-website-tc.pdf">terms of service</a>.
                </Text>
                <GeneralButton
                  type="primary"
