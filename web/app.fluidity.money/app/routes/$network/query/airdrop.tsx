...
export const dayDifference = (date1: Date, date2: Date) => Math.ceil(Math.abs(date1.getTime() - date2.getTime()) / 1000 / 60 / 60 / 24)
// total - (now - start)
const EpochDaysRemaining = () => EPOCH_DAYS_TOTAL - dayDifference(new Date(), EPOCH_START_DATE)
...
export const loader: LoaderFunction = async ({ params, request }) => {
  ...
  try {
    const {data: airdropStatsData, errors: airdropStatsErrors} = await useAirdropStatsByAddress(address);
    const {data: leaderboardData, errors: leaderboardErrors} = await useAirdropLeaderboard();
    const {data: stakingData, errors: stakingErrors}= await useStakingDataByAddress(network, address);

    if (!stakingData)
      throw stakingErrors;
    if (!airdropStatsData)
      throw airdropStatsErrors;
    if (!leaderboardData)
      throw leaderboardErrors;

    const {lootboxCounts: bottleCounts, liquidityMultiplier: {result: liquidityMultiplier}, referralsCount: {aggregate: {count: referralsCount}}} = airdropStatsData;
