...
const contractTarget = (address: string, action: "wrap" | "unwrap") =>
  `${address}::fluidity_coin::${action}` as const;
...
const internalSwap = async (
  ...
  fromToken: Token,
  toToken: Token,
  signAndExecuteTransactionBlock: ReturnType<
  ...
  if (balance.lt(amountBig)) {
    throw new Error(
      `balance was not sufficient! wanted ${amountBig.toString()}, had ${balance.toString()}`
    );
  }
  ...
  if (i > 1) {
    txb.mergeCoins(
      firstCoinId,
      coins.slice(1, i).map((coin) => coin.coinObjectId)
    );
  }

  // look up the required accounts
  const key = (
    fromToken.symbol.startsWith("f") ? toToken.symbol : fromToken.symbol
  ) as keyof typeof suiAddresses.mainnet;
...
export { internalSwap, getBalance };
