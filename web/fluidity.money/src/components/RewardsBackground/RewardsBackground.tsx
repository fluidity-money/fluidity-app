...
const RewardsBackground = () => {
  ...
  const [carouselInfo, setCarouselInfo] = useState<JSX.Element>(undefined)
  ...
  const [rewardCarousels, setRewardCarousels] = useState<JSX.Element[]>(undefined);

  useEffect(() => {
    setRewardCarousels(
      Array.from({length: rewardLimit}).map((_, i) => (
        <div key={`reward-carousel-${i}`}>
          <motion.div
