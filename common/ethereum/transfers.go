...
func GetApplicationTransfers(logs []ethereum.Log, transactions []ethereum.Transaction, blockHash ethereum.Hash, applicationContracts map[string]struct{}, classifyApplicationLogTopic func(topic string) applications.Application) ([]worker.EthereumApplicationTransfer, error) {
	blockTransactions := make(map[ethereum.Hash]ethereum.Transaction)
...
func IsApplicationContract(address string, contracts map[string]struct{}) bool {
	_, exists := contracts[address]
	return exists
}
