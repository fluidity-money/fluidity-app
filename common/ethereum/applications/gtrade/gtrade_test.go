...
func TestGetGtradeV6_1Fees(t *testing.T) {
	const (
	...
	// nil transfer fails
	fees, err := GetGtradeV6_1Fees(transfer, client, fluidTokenAddr, tokenDecimals)
	assert.Error(t, err)
	assert.Nil(t, fees)
	...
	transfer.Log.Topics = []ethereum.Hash{
		ethereum.HashFromString(gtradeV6_1SwapLogTopic),
	}
	fees, err = GetGtradeV6_1Fees(transfer, client, fluidTokenAddr, tokenDecimals)
	assert.Error(t, err)
	assert.Nil(t, fees)
	...
	fees, err = GetGtradeV6_1Fees(transfer, client, fluidTokenAddr2, tokenDecimals)
	assert.NoError(t, err)
	assert.Equal(t, r, fees)
