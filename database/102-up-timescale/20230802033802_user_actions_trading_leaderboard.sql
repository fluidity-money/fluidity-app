...
CREATE FUNCTION trading_leaderboard(network_ network_blockchain, start_time TIMESTAMP, end_time TIMESTAMP, application_ VARCHAR[] DEFAULT NULL, address_ VARCHAR DEFAULT NULL)
...
WHERE
  ...
  AND time >= start_time
  AND time < end_time
	AND (application_ IS NULL OR (application = ANY (application_)))
	AND (address_ IS NULL OR (sender_address = address_))
